...
<script type="text/javascript">
	...
	function query(obj) {
		...
		//为标签赋值
		//document.getElementById("proid").value = obj;

		//搜索团队，列表
		//$("#shows").parent().nextAll().remove();
		$("#shows").html("");//清空指定元素内容
		if (obj != 0) {
			$
					.post(
							"/project_management/PlanController/showall.action",
							{
								proid : obj
							...
							function(data) {
								if (data != null && data.length > 0) {
									...
									for ( var item in data) {
										...
										if (data[item].state == 0)
											con = "<label class='badge badge-gradient-secondary'>未开始</label>";
										else if (data[item].state == 1)
											con = "<label class='badge badge-gradient-danger'>已结束</label>";
										else if (data[item].state == 2)
											con = "<label class='badge badge-gradient-info'>进行中</label>";
										else if (data[item].state == 3)
											con = "<label class='badge badge-gradient-warning'>已失效</label>";
										...
										content += "<tr><td>"
												...
												+ "</td>"
												+ "<td>"
												+ "<a class='text-primary' href='javascript:void(0)' onclick='queryDay("+data[item].planid+")'>变更工期</a>"
												+ "</td>" + str + "</tr>";
									}
									$("#shows").append(content);
								} else {
									content = "<br><br><br><br><br> <center> <p class='text-primary'>别担心。<p >添加项目计划。</p></center>";
									$("#shows").append(content);
								}
							}, "json");
		}
	...
	function queryDay(planid) {
	    //alert('ajax请求之前');
	    $.ajax({
	        url : "/project_management/PlanController/getPlan/"+planid+".action",
	        async : true,
	        type : "GET",
	        // 成功后开启模态框
	        success : showQuery,
				...
				<div class="content-wrapper">
					...
					<div class="page-header">
						<h3 class="page-title">
							...
							</span> <a href="#" class="text-primary">项目计划管理</a> / 计划变更 <br> <br>
							<select class="form-control" onchange="query(this.value)">
						...
						<div class="col-sm-6">
							<c:if test="${status==0}">
								<div class="alert alert-success">
									<a href="#" class="close" data-dismiss="alert"> &times; </a> <strong>成功！</strong>工期更改完成
								</div>
							</c:if>
							<c:if test="${status==1}">
												...
												<c:forEach items="${planlist }" var="item">
													<tr>
														...
														<td><a class="text-primary" href="javascript:void(0)"
															onclick="query(${item.planid })">变更工期</a></td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
							...
							<div class="modal-content" style="width: 750px;">
								<div class="card">
									...
									<form role="form" method="post"
										action="/project_management/PlanController/updateTime.action">
										<div class="modal-body">
											<!-- 文字 -->
											<div class="row">
												<div class="col-md-6">
													<div class="form-group row">
														<label class="col-sm-4 col-form-label">计划ID</label>
														<div class="col-sm-8">
															<input type="text" class="form-control border-0"
																id="planid" name="planid" readonly />
														</div>
													</div>
												</div>
												<div class="col-md-6">
													<div class="form-group row">
														<label class="col-sm-4 col-form-label">计划名称</label>
														<div class="col-sm-8">
															<input type="text" class="form-control border-0"
											...
											<div class="row">
												<div class="col-md-6">
													<div class="form-group row">
														<label class="col-sm-4 col-form-label">工期</label>
														<div class="col-sm-8">
															<input type="number" class="form-control" id="newdate" name="newdate"/>
														</div>
													</div>
												</div>
											</div>
											...
											<div class="modal-footer">
												<button type="submit"
													onclick="return confirm('修改？一经修改无法撤回');"
													class="btn btn-gradient-primary mr-2">修改</button>
